...
def _dig_dict(body: dict, path: list[str]) -> dict:
    """Like `dig`, but for paths known (by schema) to be containers.

    `dig` returns `str | dict | None` because it can walk to any node,
    including a leaf. These call sites are always addressing a branch node
    (e.g. System/Config, a zone's HDMI/Output) that's guaranteed by the
    protocol's own structure to be a dict whenever it's present -- this
    narrows that back to `dict` for type-checking without weakening `dig`
    itself, which still needs to allow leaves for its other callers.
    """
    node = dig(body, path)
...
class DeviceInfo:
    ...
    def from_response(cls, body: dict) -> DeviceInfo:
        config = _dig_dict(body, ["System", "Config"])
        ...
        input_names = _dig_dict(body, ["System", "Config", "Name", "Input"])
        return cls(
...
class ZoneCapabilities:
    ...
    def from_response(cls, body: dict) -> ZoneCapabilities:
        ...
        scenes = _dig_dict(body, ["Config", "Name", "Scene"])
        return cls(
...
class ZoneStatus:
    ...
    def from_response(cls, body: dict) -> ZoneStatus:
        ...
        program_current = _dig_dict(surround, ["Program_Sel", "Current"])
        ...
        hdmi_out = _dig_dict(sound_video, ["HDMI", "Output"])
