...
A second, sibling integration in this repo, living at
`custom_components/yamaha_ync/` next to the main `yamaha_ynca` integration
above. Built on Yamaha's officially-documented **YNC** protocol
(`/YamahaRemoteControl/ctrl`, HTTP/XML) -- a completely different wire
protocol from `yamaha_ynca`'s YNCA (TCP 50000) -- for receivers where YNC is
the better (or only) fit. Built and validated against a Yamaha RX-A3080
(AVENTAGE, firmware 2.16/3.14), which the table above already lists as a
working `yamaha_ynca` model too; the two protocols aren't mutually
exclusive on a given unit.
...
lock). `tests/yamaha_ync/` has 28 passing tests covering the protocol/model/
client/notify-parsing layer against real captured payloads (including the
...
`tests/yamaha_ync/test_ha_integration.py` covers config-flow (user/error/
duplicate/reconfigure) and setup/unload (asserting the `remote` platform and
its activity list too) against a real `hass` fixture -- 5 more tests, **33
total**, verified against this repo's existing CI
(`.github/workflows/validations.yaml`, `ubuntu-latest`), which already runs
`pytest` + `mypy` + hassfest over the whole tree. It can't run locally on
native Windows: `pytest-homeassistant-custom-component`'s event loop policy
needs a real loopback socket to even construct itself, which collides with
`pytest-socket`'s default network block (a well-documented Windows/asyncio
ProactorEventLoop quirk, not something specific to this integration). Before
this integration joined the repo, an interim CI workflow of its own caught a
real reconfigure-flow bug (comparing a still-unset unique_id) along with
several CI/test-plumbing issues (missing test dependency, faked-setup
teardown crashing on unset `runtime_data`, a `pycares` DNS-resolver thread
lazily spun up by a real `aiohttp.ClientSession` the harness's thread-leak
check has no opt-out for) -- worth knowing the history even though that
workflow file itself didn't come along into this repo, superseded by the
one already here.
...
- Not registered in `home-assistant/brands` yet (Bronze `brands` rule) --
  a separate registration from `yamaha_ynca`'s own, since it's a distinct
  `domain`.
- Not yet independently published -- currently only reachable by installing
  this whole repo (HACS custom repository, or copying
  `custom_components/yamaha_ync/` into `config/custom_components/`
  manually); hasn't been evaluated for whether it belongs as a second
  domain in the same HACS listing as `yamaha_ynca` or as its own repo.

## Development

This integration's dev/test tooling is folded into the repo's existing
`pyproject.toml` -- no separate install step:

```bash
pip install -e ".[dev]"
pytest tests/yamaha_ync/ -v          # this integration's tests only; needs Linux for test_ha_integration.py
python scripts/probe_device.py <receiver-ip>   # read-only, safe against real hardware
```

## Blueprints

`blueprints/automation/` (inside this integration's own directory -- kept
out of the repo root since a top-level `blueprints/` directory trips up
this project's `setuptools` package auto-discovery) has two starting points
