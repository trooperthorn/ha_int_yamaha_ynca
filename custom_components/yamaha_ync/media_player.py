...
class YncMediaPlayer(YncZoneEntity, MediaPlayerEntity):
    ...
    def _netusb_source(self) -> str | None:
        """The zone's current input, if it's a netusb-family source.

        A method rather than a bool property so the three transport-control
        call sites below get back an already-narrowed `str`, not a `str |
        None` they'd need to re-check.
        """
        input_id = self._zone.status.input_id
        return input_id if input_id in NETUSB_FAMILY_INPUTS else None
    ...
    async def _async_playback(self, command: str) -> None:
        source = self._netusb_source()
        if source is None:
            return
        await self.coordinator.client.netusb_playback(source, command)
        await self.coordinator.async_request_refresh()
    ...
    async def async_set_shuffle(self, shuffle: bool) -> None:
        ...
        if source is None:
            return
        await self.coordinator.client.netusb_set_shuffle(source, shuffle)
        await self.coordinator.async_request_refresh()
    ...
    async def async_set_repeat(self, repeat: RepeatMode) -> None:
        ...
        if device_value is None:
            return
        await self.coordinator.client.netusb_set_repeat(source, device_value)
        await self.coordinator.async_request_refresh()
